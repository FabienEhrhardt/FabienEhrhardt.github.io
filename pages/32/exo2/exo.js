...
const I1mA_choices = [500, 600, 800, 1000, 1200, 1500, 2000, 2500, 3000];
const I1mA = I1mA_choices[Math.floor(Math.random() * I1mA_choices.length)]; // en mA
const I1 = I1mA / 1000; // en A
...
const Ieff = I1 * Math.sqrt(sumHsq); // en A
const IeffmA = Math.round(Ieff * 1000); // en mA
...
const ImaxmA = Math.round(Imax * 1000);
...
const IadmA = Math.round(Iad * 1000);
...
const val1mA = Math.round(I1mA * harmonics[rang1] / 100);

const rang2 = top0[1];
const freq2 = rang2 * freq;
const val2mA = Math.round(I1mA * harmonics[rang2] / 100);
...
var I1_spec = I1mA; // mA, utilisé par SpectreCA
var THD_spec = THDarrondi;
var Iad_spec = IadmA / 1000; // A (P10 attend des A)
...
function drawEcran1(canvasId) {
  ...
  ctx.fillStyle = "#ffaa00";
  ctx.fillText(`A=${I_cur}mA`, gx + 180, 380);

  // Légende
  ctx.fillStyle = "#00ff44";
  ctx.fillText("▬ Tension (V)", gx + gw - 200, 70);
  ctx.fillStyle = "#ffaa00";
  ctx.fillText("▬ Courant (mA)", gx + gw - 70, 70);
}
...
function drawEcran2(canvasId) {
  ...
  ctx.fillStyle = "#00ff44";
  ctx.fillText(I1mA + " mA", 280, 55);
  ctx.fillText("+000°", 430, 55);
  ctx.fillStyle = "#ff6666";
  ctx.fillText("THD  " + THDarrondi.toFixed(1) + " %", 430, 80);
  ctx.fillText("Ad   " + IadmA + " mA", 430, 100);
...
function drawEcran3(canvasId) {
  ...
  for (let i = 0; i < vals.length; i++) {
    ...
    const txt = sign + (absv >= 1000 ? (absv / 1000).toFixed(3) + "k" : absv.toFixed(0));
...
let exo = {
  ...
  titre: "Analyse d'un réseau avec harmoniques de courant – Lecture",
  ...
  questions: [
    ...
    // ===================== Q1 – Lecture Veff
    {
      texte: `<b>Écran 1 – Formes d'ondes.</b><br>
      Relever la valeur efficace de la <span style="color:#00aa00">tension</span> affichée sur l'analyseur.`,
      reponse: Veff,
      tolerance: 0.02,
      unite: "V",
      feedback: `L'écran 1 affiche en haut à gauche la valeur efficace RMS : <b>${Veff} V</b>.`
    },

    // ===================== Q2 – Lecture Vpeak
    {
      texte: `<b>Écran 1 – Formes d'ondes.</b><br>
      Relever la valeur de crête (maximale) de la <span style="color:#00aa00">tension</span> sur l'écran.`,
      reponse: Math.round(Vmax),
      tolerance: 0.03,
      unite: "V",
      feedback: `Le curseur (en pointillé) est placé sur la valeur maximale de la tension ici. La valeur maximale mesurée sur l'écran est visible sur l'axe gauche de l'écran 1 : <b>${Math.round(Vmax)} V</b>.<br>
      On peut aussi la calculer car V est sinusoïdal: \\(V_{max} = V_{eff}\\times\\sqrt{2} = ${Veff}\\times\\sqrt{2} \\approx ${Math.round(Vmax)}~V\\)`
    },

    // ===================== Q3 – Lecture Ieff
    {
      texte: `<b>Écran 1 – Formes d'ondes.</b><br>
      Relever la valeur efficace du <span style="color:#cc7700">courant</span> affichée sur l'analyseur. Donner la réponse en mA.`,
      reponse: IeffmA,
      tolerance: 0.03,
      unite: "mA",
      feedback: `L'écran 1 affiche la valeur efficace du courant : <b>${IeffmA} mA</b>.`
    },
	
	// ===================== Q4 – Lecture Imax
    {
      texte: `<b>Écran 1 – Formes d'ondes.</b><br>
      Relever la valeur maximale du <span style="color:#cc7700">courant</span> affichée sur l'analyseur. Donner la réponse en mA.`,
      reponse: ImaxmA,
      tolerance: 0.03,
      unite: "mA",
      feedback: `L'écran 1 affiche la variation de la valeur instantanée  du courant. L'échelle est donnée à gauche de la courbe, il faut donc mesurer, avec le curseur ou avec la règle la valeur. \\(I_{max}=\\)<b>${arrondi(ImaxmA)} mA</b>.<br>
      Ce courant contient des harmoniques, donc \\(I_{eff} \\ne \\frac{I_{max}}{\\sqrt{2}}\\).`
    },

    // ===================== Q5 – Lecture fondamental sur spectre
    {
      texte: `<b>Écran 2 – Spectre harmonique.</b><br>
      Relever la valeur efficace du courant fondamental (rang 1) indiquée en haut de l'écran 2. Donner la réponse en mA.`,
      reponse: I1mA,
      tolerance: 0.02,
      unite: "mA",
      feedback: `L'écran 2 affiche en haut : <b>"A-h01 → ${I1mA} mA"</b>.<br>
      C'est la valeur efficace du fondamental seul (rang 1, fréquence ${freq} Hz).
	  Ce courant contient des harmoniques, donc \\(I_{eff} > I_1 = ${I1mA}~mA\\).`
    },

    // ===================== Q6 – Rang harmonique dominant
    {
      texte: `<b>Écran 2 – Spectre harmonique.</b><br>
      Quel est le rang de l'harmonique de courant le plus important (hors fondamental) ?`,
      reponse: rang1,
      tolerance: 0.01,
      unite: "",
      feedback: `Le rang ${rang1} est la barre la plus haute du spectre après le fondamental.<br>
      Sa fréquence est : \\(f_{${rang1}} = ${rang1} \\times ${freq} = ${freq1}~Hz\\)<br>
      Sa valeur efficace est : \\(I_{${rang1}} = ${harmonics[rang1]}\\% \\times ${I1mA} = ${val1mA}~mA\\)`
    },

    // ===================== Q8 – Lecture THD
    {
      texte: `<b>Écran 2 – Spectre harmonique.</b><br>
      Relever le taux de distorsion harmonique en courant (THD) affiché sur l'analyseur.`,
      reponse: THDarrondi,
      tolerance: 0.05,
      unite: "%",
      feedback: `L'écran 2 affiche : <b>THD = ${THDarrondi} %</b>.<br>
      Le THD mesure le rapport entre les harmoniques et le fondamental :<br>
      \\(THD = \\frac{I_{ad}}{I_1} \\times 100 = \\frac{${IadmA}}{${I1mA}} \\times 100 \\approx ${THDarrondi}~\\%\\)`
    },


    // ===================== Q9 – Lecture puissance active P
    {
      texte: `<b>Écran 3 – Puissances.</b><br>
      Relever la puissance active consommée par la charge.`,
      reponse: Math.abs(P_p),
      tolerance: 0.03,
      unite: "W",
      feedback: `L'écran 3 affiche : <b>P = ${P_p >= 0 ? "+" : ""}${P_p} W</b>.<br>
      La puissance active est la seule vraiment "utile" (travail mécanique, chaleur...).`
    },

    // ===================== Q10 – Calcul puissance déformante D
    {
      texte: `Relever la puissance déformante à partir des valeurs relevées sur l'écran.`,
      reponse: D_rep,
      tolerance: 0.05,
      unite: "vad",
      feedback: `L'écran 3 affiche la puissance déformante <b>D = ${D_p} vad</b>`
    },

    // ===================== Q11 – Calcul puissance apparente S
    {
      texte: `Relever la puissance apparente totale.`,
      reponse: S_rep,
      tolerance: 0.05,
      unite: "VA",
      feedback: `L'écran 3 affiche la puissance apparente <b>S = ${S_p} VA</b>`
    },

    // ===================== Q12 – Facteur de puissance
    {
      texte: `Donner la valeur du facteur de puissance de l'installation.`,
      reponse: PF_rep,
      tolerance: 0.05,
      unite: "",
      feedback: `L'écran 3 affiche le facteur de puissance <b>PF = ${PF_p}</b> <br>
