function tracerEcran(ecran) {
  ...
  // Échelles
  const scaleX = (W - 80) / (xmax-xmin); // pixels par seconde
  const scaleY1 = (H-60)/2/ymax1; // pixels par unité 1
  const scaleY2 = (H-60)/2/ymax2; //pixels par unité 2
  ...
  // Verticales
  for (let x = ox; x < W; x += scaleX*(xmax-xmin) / 3) {
    ctx.beginPath();
    ctx.moveTo(x, 20);
    ctx.lineTo(x, H - 20);
    ctx.stroke();
  ...
  // Axe x
  ctx.fillText(TitreAxeX, W / 2 - 40, H - 25);
  ctx.fillStyle = "black";
  ctx.fillText(`${xmax}`, W-40, oy-5);
  ctx.fillText(`${xmin}`, 50, oy-5);
  ctx.save();
...
function tracerCourbe(ecran, fonction, NomCourbe, couleur, numero ) {
  ...
  for (let t = xmin; t <= xmax; t += pas) {
    let x = ox + (t-xmin) * scaleX;
